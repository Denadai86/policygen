...
import { 
  ...
  ArrowLeft, Check, Copy, Save, LogIn, RefreshCw, AlertCircle
} from "lucide-react";
...
function FinalPageContent() {
  ...
  // Lista FILTRADA de documentos que REALMENTE foram gerados (não são string vazia)
  ...
  // 3. SALVAR OU ATUALIZAR (Lógica de Ouro)
  ...
  const generate = async () => {
    ...
    setError(null);
    setDocs(null);
    setActiveTab(null); // Reseta a aba ativa
    try {
        ...
        if (json.documents) {
            ...
            // O availableDocs e o useEffect cuidarão de setar a aba ativa
        } else {
            setError("A IA não retornou documentos válidos.");
        }
    } catch (err: any) {
  ...
  const downloadAll = () => {
      if(!docs) return;
      // Baixa apenas os que foram gerados
      availableDocs.forEach(doc => {
        downloadFile(doc.content, `${doc.key}.md`);
      });
  };
  ...
  const handleSaveGist = async () => {
       ...
       try {
           const result = await saveToGist(`${activeTab}.md`, currentDoc.content);
  ...
  const currentDocContent = activeTab && availableDocs.find(d => d.key === activeTab)?.content;
    ...
    <div className="max-w-6xl mx-auto pt-16 pb-24 px-6 relative min-h-screen flex flex-col">
      ...
      <div className="relative z-10 text-center mb-10">
        ...
        {docs && availableDocs.length > 0 && (
            <div className="mt-6 flex justify-center animate-in fade-in slide-in-from-bottom-4">
                {/* Lógica de Autenticação e Salvamento */}
                {!user ? (
                    <button 
                        onClick={handleLogin}
      ...
      {(!docs || availableDocs.length === 0) && !loading && (
        <div className="relative z-10 flex flex-col sm:flex-row justify-center gap-4">
          ...
          {error && <div className="mt-4 p-3 bg-red-900/20 border border-red-500/30 text-red-200 rounded-lg flex items-center gap-2"><AlertCircle size={18}/> {error}</div>}
        </div>
      )}
