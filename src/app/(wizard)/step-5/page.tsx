...
function Step5Content() { // Componente wrapper para Suspense
  const router = useRouter();
  const searchParams = useSearchParams();
  const projectId = searchParams.get("projectId"); // <-- Lendo o ID
  const { data, update } = useWizard();

  // ============================================
  // STATES (Todos vindos do contexto/hidratação)
  // ============================================
  ...
  const nextStep = () => {
    // 1. Salva todos os estados no Contexto
    ...
    // 2. Navega para o próximo passo, MANTENDO O ID
    const query = projectId ? `?projectId=${projectId}` : "";
    router.push(`/step-6${query}`); 
  };

  const handleBack = () => {
    // 1. Navega para o passo anterior, MANTENDO O ID
    const query = projectId ? `?projectId=${projectId}` : "";
  ...
  return (
    <div className="max-w-4xl mx-auto py-16 px-6 relative">
      ...
      <div className="space-y-10 pb-20">
        {/* SEU CÓDIGO DE COOKIES COMEÇA AQUI */}
        ...
        {usesCookies && (
          <>
            ...
            <section className="card bg-white/5 border border-white/10 p-6 rounded-2xl">
               ...
               <textarea rows={3} className="w-full p-3 rounded-xl bg-black/30 border border-white/10 text-white" placeholder="Por quanto tempo os dados são mantidos?" value={retentionPolicy} onChange={e => setRetentionPolicy(e.target.value)} />
            </section>
          </>
        )}
        ...
        <div className="flex justify-between pt-4">
          <button
            onClick={handleBack}
            className="px-6 py-3 bg-white/5 border border-white/10 rounded-xl text-white flex items-center gap-2"
          >
